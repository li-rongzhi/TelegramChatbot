...
async def style_transfer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "You are now in the style transfer session, you can upload a file and choose a style. Use /back to return to General mode."
    )
...
async def style(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if db.check_user_state(user_id, STYLE_TRANSFER):
        ...
        await update.message.reply_text("Please start a Style Transfer session using /style_transfer.")
